...
#define OBJECT_STORAGE_QUEUE_RELATED_SETTINGS(DECLARE, ALIAS) \
    ...
    DECLARE(UInt32, processing_state_cache_ttl_seconds, 5 * 60, "How long a `Processing` file state which was read from keeper, i.e. the file is held by another processor on this or another server, is trusted without rechecking keeper. Zero means to always recheck keeper. A `Processing` state set by a processor of this server is not affected: its owner updates it itself", 0) \
